 ...
 * @LastEditTime: 2021-03-11 11:23:11
 ...
 */
const setCookie = (cookieName, value, overtime = 1) => Cookies.set(cookieName, value, { expires: hasNumber(overtime) ? overtime : 1 });
 ...
 */
const getToken = () => getCookie(TOKENKEY);
 ...
 */
const setToken = (token, overtime) => setCookie(TOKENKEY, token, overtime);

/**
 * @description: 删除token
 */
const delToken = () => delCookie(TOKENKEY);
 ...
 */
const shortCacheGet = (name) => JSON.parse(sessionStorage.getItem(name)) || false;
 ...
 */
const shortCacheSet = (name, jsonObject) => sessionStorage.setItem(name, JSON.stringify(jsonObject));

/**
 * @description: 短期缓存删除
 * @param {String | Null} name 缓存名称
 */
const shortCacheDel = (name = null) => {
 ...
 * @return {* | Boolean} 获取的缓存值，如果没有则返回 false
 */
const cacheGet = (name) => JSON.parse(localStorage.getItem(name)) || false;
 ...
 * @param {*} jsonObject 缓存值
 */
const cacheSet = (name, jsonObject) => localStorage.setItem(name, JSON.stringify(jsonObject));

/**
 * @description: 缓存删除
 * @param {String | Null} name 缓存名称
 */
const cacheDel = (name = null) => {
    if (name === null || hasString(name)) {
